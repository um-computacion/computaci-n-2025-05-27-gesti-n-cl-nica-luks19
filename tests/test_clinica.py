...
class TestClinica(unittest.TestCase):
    def setUp(self):
        ...
        self.medico = Medico("Dr. Smith", "MP-12345", ["Cardiología"])
        self.clinica.agregar_paciente(self.paciente)
        self.clinica.agregar_medico(self.medico)
        self.especialidad = Especialidad("Cardiología", ["lunes"])
        if hasattr(self.clinica, "agregar_especialidad_a_medico"):
    ...
    def test_agregar_medico_duplicado(self):
        m = Medico("Dr. House", "MP-12345", ["Clínica"])
        with self.assertRaises(MedicoExistenteException):
            self.clinica.agregar_medico(m)

    def test_agendar_turno_valido(self):
        fecha = datetime.strptime("16/06/2099 10:00", "%d/%m/%Y %H:%M")
